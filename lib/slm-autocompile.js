  ...
  function getOptions(file, options) {
    ...
    if (params) {
      ...
      for (var i = 0; i < opts.length; i++) {
        ...
        options[name] = value;
        if (name == 'self' || name == 'debug' || name == 'compileDebug' || name == 'pretty') {
  ...
  module.exports = {
    ...
    compile: function compile(file, cb) {
      ...
      fs.readFile(file, {encoding: 'utf-8'}, function readFile(err, data) {
        ...
        //Compile "main" file(s)
        if (options.main) {
          let mainPaths = options.main.split(' ');

          for (let mainPath of mainPaths) {
            let uri = path.resolve(path.dirname(file), mainPath.trim());

            //Prevent infinite loop
            if (uri === file) continue;

            _this.compile(uri).then(function (err, _slm, options) {
              if (err){
                var discomposed = err.message.split('\n');
                atom.notifications.addWarning(discomposed[0] + '\n' + discomposed[discomposed.length - 1], {dismissable: true, detail: discomposed.slice(1, discomposed.length - 2).join('\n')});
                if (_this.activeEditor)
              ...
              _this.saveSlm(uri, _slm, options.output, function() {
        ...
        //Set cache to false to force reimporting of files
